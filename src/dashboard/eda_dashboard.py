...
# table in seconds and decide who to call that day.
    ...
    def load_predictions():
        ...
        # I attach full student names from the cleaned students parquet so the
        # moderator sees "Malika Yusupova" rather than a 24-character MongoDB
        # ObjectId. The name_col fallback handles any future column rename.
        if Path(STUDENTS_CLEAN).exists():
            stu = pd.read_parquet(STUDENTS_CLEAN)
            name_col = 'fullName' if 'fullName' in stu.columns else stu.columns[0]
            stu_cols = ['studentId', name_col]
            if 'phoneNumber' in stu.columns:
                stu_cols.append('phoneNumber')
            pred = pred.merge(
                stu[stu_cols].rename(columns={name_col: 'studentName'}),
